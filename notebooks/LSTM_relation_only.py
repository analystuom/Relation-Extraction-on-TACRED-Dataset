...
# filter out records with "no_relation" class
def filter_no_relation(data):
    filtered_data = [sample for sample in data if sample["relation"] != "no_relation"]
    print(
        f"Filtered data: {len(filtered_data)} samples (removed {len(data) - len(filtered_data)} 'no_relation' samples)")
...
# additional function to prepare data for batch processing in Pytorch's DataLoader
...
def train_and_evaluate(train_data, dev_data, test_data=None, batch_size=32, epochs=20,
                       lr=0.001, max_len=60, embedding_dim=200, hidden_dim=256, dropout_rate=0.3):
    # Filter out no_relation samples from all datasets
    filtered_train_data = filter_no_relation(train_data)
    filtered_dev_data = filter_no_relation(dev_data)
    filtered_test_data = filter_no_relation(test_data) if test_data is not None else None

    word_vocab = build_vocab(filtered_train_data)
    pos_vocab, ner_vocab = build_pos_ner_vocabs(filtered_train_data)
    entity_type_vocab = build_entity_type_vocab(filtered_train_data)
    ...
    processed_train = preprocess_data(filtered_train_data, word_vocab, pos_vocab, ner_vocab, entity_type_vocab, max_len)

    train_dataset = RelationDataset(processed_train)
    label_encoder = train_dataset.label_encoder
    processed_dev = preprocess_data(filtered_dev_data, word_vocab, pos_vocab, ner_vocab, entity_type_vocab, max_len)
    ...
    torch.save(save_dict, "../models_parameters/re_lstm_model_no_relation_filtered.pt")

    # evaluate on test data
    if filtered_test_data:
        processed_test = preprocess_data(filtered_test_data, word_vocab, pos_vocab, ner_vocab, entity_type_vocab,
                                         max_len)
...
if __name__ == "__main__":
    ...
    # Check if we want to load an existing filtered model
    model_path = "../models_parameters/re_lstm_model_no_relation_filtered.pt"

    if os.path.exists(model_path):
        print("Loading existing filtered model...")
        model, word_vocab, pos_vocab, ner_vocab, entity_type_vocab, label_encoder, max_len = load_model(model_path)

        # evaluate on filtered test set
        if os.path.exists(test_filepath):
            print("\nEvaluating on filtered test set...")
            test_data = load_data(test_filepath)
            filtered_test_data = filter_no_relation(test_data)

            if len(filtered_test_data) > 0:
                processed_test = preprocess_data(filtered_test_data, word_vocab, pos_vocab, ner_vocab,
                                                 entity_type_vocab, max_len)
                test_dataset = RelationDataset(processed_test, label_encoder)
                ...
                with torch.no_grad():
                    for batch in test_loader:
                        ...
                        result_df.to_csv("predictionvector_lstm_relation.csv", index=False)
                ...
                print("No valid test samples after filtering out 'no_relation' samples.")
    else:
        print("Training new model - LSTM (no_relation filtered)")
